...
- Bring-your-own-key model credentials: one AES-GCM encrypted key per provider per organization (`PUT`/`GET`/`DELETE /api/ai-credentials`), never returned by any endpoint, managed from a new section on the existing Settings screen. A **Test** action (`POST /api/ai-credentials/:provider/test`) makes one minimal structured call and reports which model answered and what it cost — proving the key, the model id and structured-output support in a single act, never from a cache. Removing a key fails that organization's queued generation runs with a message naming the missing key instead of leaving them to die on a provider 401. Settings also shows the organization's spend to date, summed by `org_id` alone and rendered under three rules that stop a nullable `SUM()` from printing a confident wrong number: any unpriced call makes the figure a floor (`≥ $X`), any locally-estimated call makes it approximate (`≈ $X`), and only provider-reported costs make it exact.
