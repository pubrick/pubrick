...
describe.skipIf(!url)("publication analytics e2e", () => {
  ...
  it("queues only an owned publication sample and retains an earlier aggregate after refresh", async () => {
    const owner = await orgAgent();
    const other = await orgAgent();
    ...
    expect((await owner.agent.get(path).expect(200)).body).toMatchObject({ status: "no_key" });
    await other.agent.get(path).expect(404);
    ...
    await owner.agent
      ...
      .send({
        provider: "google",
        apiKey: "publication-test-key-never-used",
      })
      .expect(200);
    expect((await owner.agent.get(path).expect(200)).body).toMatchObject({
      status: "not_analyzed",
    });
    const countTokens = vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
      if (!String(input).endsWith(":countTokens")) throw new Error("Unexpected generation request");
      expect(init?.method).toBe("POST");
      expect(String(init?.body)).toContain("How does the small team price work?");
      return new Response(JSON.stringify({ totalTokens: 200 }), { status: 200 });
    });
    const queued = commentAnalysisDtoSchema.parse((await owner.agent.post(path).expect(200)).body);
    expect(queued).toMatchObject({
      status: "in_progress",
      current: { sampleVersion, collectionStatus: "available" },
    });
    expect(JSON.stringify(queued)).not.toContain("publication-test-key-never-used");
    expect((await owner.agent.post(path).expect(200)).body).toMatchObject({
      status: "in_progress",
    });
    ...
    const attempts = await db
      ...
      .where(
        and(
          eq(schema.paidReplyAnalysisAttempts.orgId, owner.orgId),
          eq(schema.paidReplyAnalysisAttempts.targetId, publication.id),
        ),
      );
    ...
    expect(
      await db.select().from(schema.usageLedger).where(eq(schema.usageLedger.orgId, owner.orgId)),
    ).toHaveLength(0);
    ...
    await db.insert(schema.publicationCommentAnalyses).values({
      orgId: owner.orgId,
      brandId: brand.body.id,
      publicationId: publication.id,
    ...
    await db
      ...
      .set({
        ...
        sampleVersion: newVersion,
      })
      .where(eq(schema.publicationCommentSamples.publicationId, publication.id));
    expect((await owner.agent.get(path).expect(200)).body).toMatchObject({
      status: "stale",
      current: { sampleVersion: newVersion },
      earlierAnalysis: { sampleVersion, sampleSize: 1, result },
    });
    expect((await owner.agent.post(path).expect(200)).body).toMatchObject({
      status: "in_progress",
    });
