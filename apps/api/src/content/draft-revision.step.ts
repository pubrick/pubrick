...
export type DraftRevisionInput = { title: string | null; body: string; instruction: string };
...
export function draftRevisionStep(): Step<DraftRevisionInput, DraftRevisionOutput, RefineContext> {
  return defineStep({
    ...
    schema: outputSchema,
    role: [
      "You revise the title and entire master draft of a social media post for a human editor.",
      ...
      "Return title as the complete replacement title (or null only if the post should remain untitled), text as the complete replacement post in the draft's language, and reason as one brief sentence explaining the main changes.",
    ],
    material: (_context, input) => [
      { label: "EDITOR INSTRUCTION", text: input.instruction },
      { label: "SAVED TITLE (untrusted content)", text: input.title ?? "(untitled)" },
