...
Pubrick can generate a social post, news digest, source retelling, product update, expert article, or how-to guide. Choose **Generate as** on the New post screen. Source retelling requires pasted or explicitly accepted source text; other formats can start from a brief, source text, or both. The selected format is saved in the run input and displayed on its receipt; retry uses the same format. Older runs with no format field remain social posts.

This ports the legacy Content Factory's `news_digest`, `repost`, `product_update`, `expert_article`, and `educational` intents into Pubrick's existing researcher → writer → editor → claims-to-verify → per-channel adapter pipeline. The legacy `repost` and `product_update` use the news digest pipeline; Pubrick gives each distinct source or release guidance within the same five-step pipeline. The format changes trusted role guidance, not the pipeline, queue, model selection, or billing path. Every physical language-model call continues through the same metered step runner. Runs remain scoped to the active organization and brand by the existing repository.
