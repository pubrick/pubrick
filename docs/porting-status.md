...
| Images and media | Brand-scoped image and MP4 upload library, manual Gemini image generation and per-image variation, opt-in Gemini covers for manual and scheduled draft runs, opt-in automatic illustrations for direct long-form runs with an approval review gate, one-attachment selection, Telegram/VK video delivery and Telegram/VK/MAX/Bluesky photo delivery; editor-managed inline image slots with safe attachment/removal and RSS snapshots | Automatic illustrations for calendar runs, one-click regeneration of a chosen inline slot, richer image provenance and later video formats |
