...
| Topics bank | Brand-scoped human-reviewed ideas, article import, AI topic suggestions from bank and scored news, manual-request semantic suppression against bounded recent reviewer-blocked titles with metered Google embeddings, approval, edit/archive, target date and priority, saved generation format and bounded editorial SEO keywords, direct generation using the approved format, and approved-topic calendar links with reviewed bulk planning | Semantic suppression on the one-call automatic path; richer topic performance feedback |
...
| Calendar | Scheduled publishing, brand calendar, brief or approved-topic slots with format and keyword snapshots plus stale-topic checks, operator-confirmed bulk planning and opt-in automatic or owner/admin-triggered placement of dated approved topics, planned draft generation and brand-scoped annual memorable-date suggestions | Recurring plans and richer scheduling signals |
