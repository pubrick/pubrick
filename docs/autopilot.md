...
The spend threshold stops **new** runs when priced usage for the brand on the
local day reaches the threshold. It is not a hard ceiling on a run already in
progress. New runs are also paused if the ledger contains an unpriced call or a
