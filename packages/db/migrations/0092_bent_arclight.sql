...
CREATE TABLE "paid_reply_analysis_attempts" (
	...
	CONSTRAINT "paid_reply_analysis_attempts_live_fields_check" CHECK ("paid_reply_analysis_attempts"."origin" = 'legacy' OR ("paid_reply_analysis_attempts"."status" <> 'legacy_consumed' AND "paid_reply_analysis_attempts"."admission_id" IS NOT NULL AND "paid_reply_analysis_attempts"."prompt_digest" IS NOT NULL AND ("paid_reply_analysis_attempts"."status" NOT IN ('queued', 'dispatching') OR "paid_reply_analysis_attempts"."prompt_encrypted" IS NOT NULL) AND "paid_reply_analysis_attempts"."sample_size" IS NOT NULL AND "paid_reply_analysis_attempts"."sample_size" BETWEEN 1 AND 30 AND "paid_reply_analysis_attempts"."model_id" IS NOT NULL AND "paid_reply_analysis_attempts"."price_window" IS NOT NULL AND "paid_reply_analysis_attempts"."admission_local_date" IS NOT NULL AND "paid_reply_analysis_attempts"."admission_timezone" IS NOT NULL AND "paid_reply_analysis_attempts"."day_start_utc" IS NOT NULL AND "paid_reply_analysis_attempts"."day_end_utc" IS NOT NULL AND "paid_reply_analysis_attempts"."day_start_utc" < "paid_reply_analysis_attempts"."day_end_utc" AND "paid_reply_analysis_attempts"."reserved_max_usd" IS NOT NULL AND "paid_reply_analysis_attempts"."reserved_max_usd" > 0 AND "paid_reply_analysis_attempts"."org_settings_revision" IS NOT NULL AND "paid_reply_analysis_attempts"."brand_threshold_revision" IS NOT NULL AND ("paid_reply_analysis_attempts"."origin" <> 'automatic' OR ("paid_reply_analysis_attempts"."free_revision" IS NOT NULL AND "paid_reply_analysis_attempts"."paid_revision" IS NOT NULL)))),
