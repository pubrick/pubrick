...
-- publication, or brand is deleted, but its frozen prompt must not. Cancel an
-- in-flight attempt before clearing its ciphertext to satisfy the queued-call
-- invariant and keep a worker from later publishing a result for a dead target.
...
BEGIN
  IF TG_TABLE_NAME = 'brands' THEN
    UPDATE paid_reply_analysis_attempts a SET
      ...
      status = CASE WHEN a.status IN ('queued', 'dispatching') THEN 'canceled' ELSE a.status END,
