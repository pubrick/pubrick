...
CREATE TABLE "news_relevance_batches" (
	...
	CONSTRAINT "news_relevance_batches_status_check" CHECK ("news_relevance_batches"."status" in ('queued', 'running', 'halting', 'completed', 'partial', 'halted')),
...
ALTER TABLE "news_relevance_batches" ADD CONSTRAINT "news_relevance_batches_brand_org_fk" FOREIGN KEY ("org_id","brand_id") REFERENCES "public"."brands"("org_id","id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE UNIQUE INDEX "news_relevance_batches_one_active_idx" ON "news_relevance_batches" USING btree ("org_id","brand_id") WHERE "news_relevance_batches"."status" IN ('queued', 'running', 'halting');--> statement-breakpoint
