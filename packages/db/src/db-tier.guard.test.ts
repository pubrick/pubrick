 ...
 *
 * The api and worker copies of this guard carry a fourth check — that the migration barrier
 * in their vitest.global-setup.ts actually migrated the database. This package has no such
 * barrier (migrate.test.ts migrates as part of what it is testing, and works on throwaway
 * databases), so there is no barrier here to hold to account.
 *
